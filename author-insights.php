...
if ($author_query->have_posts()) {
    ...
    foreach ($authors as $a) {
        ...
        if ($rep_name === $author_name || strtolower($rep_name) === $author_name_lower) {
            $author_image = $a['author_image'] ?? '';
            $author_bio = $a['author_bio'] ?? '';
            $fb_link = $a['facebook_link'] ?? '';
            $li_link = $a['linkedin_link'] ?? '';
            $found_in_repeater = true;
            break;
        }
    }

    if (!$found_in_repeater) {
            ...
            <div class="author-info-wrapper">
                <?php if ($author_image): ?>
                    ...
                    <div class="author-details">
                        ...
                        <?php else: ?>
                            <p style="color: #999;">[No bio found]</p>
                        <?php endif; ?>
                        ...
                        <?php else: ?>
                            <p style="">[No social links found]</p>
                        <?php endif; ?>
    
                        <?php if (!$found_in_repeater && ($author_image || $author_bio)): ?>
                            <small style="">(Data from single author fields, not repeater)</small>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <?php if ($posts_query->have_posts()): ?>
                ...
                <?php if ($total_posts > $initial_posts && $show_load_more): ?>
                    <div class="load-more-wrap">
                        <button class="insights-load-more-btn load-more-btn" data-offset="<?php echo esc_attr($initial_posts); ?>"
                            data-ppp="<?php echo esc_attr($load_more_increment); ?>"
                            data-author="<?php echo esc_attr($author_name); ?>" data-total="<?php echo esc_attr($total_posts); ?>">
                            Load More
                        </button>
                    </div>
                <?php endif; ?>
